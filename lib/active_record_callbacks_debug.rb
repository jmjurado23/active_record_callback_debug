...
module ActiveRecord
  module CallbacksDebug
    class << self
      ...
      def configuration
        @configuration ||= ActiveRecord::CallbacksDebug::Configuration.new
      end
    ...
    module CallTemplateExtension
      def expand(target, value, block)
        if ::ActiveRecord::CallbacksDebug.configuration.active
          result = nil
          params = {
            target: target,
            value: value,
          ...
          ::ActiveRecord::CallbacksDebug.log(params, bench)
          result
        else
...
module ActiveSupport
  module Callbacks
    if ::ActiveSupport.gem_version >= ::Gem::Version.new('5.1.0')
      class CallTemplate
        prepend ::ActiveRecord::CallbacksDebug::CallTemplateExtension
      end
    else
